...
int loadData(char *filename,Record **records,int *records_n_elem,LatLong **locations,int *locations_n_elem);
...
int main(int argc, char* argv[])
{
    ...
    Record *records=NULL;
    int records_n_elem=0;
	//std::vector<LatLong> locations;
	LatLong *locations=NULL;
	int locations_n_elem=0;
	char filename[100];
	int resultsCount=10;
    ...
    int numRecords = loadData(filename,&records,&records_n_elem,&locations,&locations_n_elem);
...
int loadData(char *filename,Record **records,int *records_n_elem,LatLong **locations,int *locations_n_elem){
	...
	while(!feof(flist)) {
	    ...
	    *locations = (LatLong *)realloc(*locations, ++*locations_n_elem * sizeof(LatLong));
	    (*locations)[*locations_n_elem-1] = latLong;
            //records.push_back(record);
	    *records = (Record *)realloc(*records, ++*records_n_elem * sizeof(Record));
	    (*records)[*records_n_elem-1] = record;
            recNum++;
        }
        fclose(fp);
    }

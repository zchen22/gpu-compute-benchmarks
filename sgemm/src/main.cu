...
extern bool writeColMajorMatrixFile(const char *fn, int, int, float *v, int v_n_elem);
...
main (int argc, char *argv[]) {
  ...
  float *matC = (float *)calloc(matArow*matBcol, sizeof(float));
  ...
  if (params->outFile) {
    ...
    cudaMemcpy(matC, dC, C_sz, cudaMemcpyDeviceToHost);
    /* Write C to file */
    pb_SwitchToTimer(&timers, pb_TimerID_IO);
    writeColMajorMatrixFile(params->outFile,
	matArow, matBcol, matC, matArow*matBcol); 
  }
